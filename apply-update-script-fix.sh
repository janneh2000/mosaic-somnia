#!/usr/bin/env bash
# Overwrites agents/src/updateGuardianMetadata.ts with a version that inlines
# the AgentRegistry.update() ABI fragment, since the SDK's curated ABI omits it.
set -euo pipefail

REPO="${REPO:-$HOME/Desktop/mosaic-somnia/mosaic}"
DEST="$REPO/agents/src/updateGuardianMetadata.ts"
...
echo "Now run it:"
echo "  cd $REPO/agents"
echo "  npx tsx --env-file=.env src/updateGuardianMetadata.ts"
